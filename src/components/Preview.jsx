...
function getBtn2Style(S) {
  return {
    fontFamily: S.bf, background:'transparent', color:S.text, border:`2px solid ${S.text}50`,
    borderRadius:S.btnRadius, transition:`all ${S.transSpeed}`,
    fontSize:'0.72rem', fontWeight:700, letterSpacing:'2px', textTransform:'uppercase',
    padding:'0.75rem 1.8rem', cursor:'pointer',
  };
}
...
function SectionLabel({ children, textColor }) {
  return (
    <div style={{
      fontSize:'0.5rem', letterSpacing:'3px', textTransform:'uppercase', fontFamily:"'DM Mono', monospace",
      opacity:0.3, display:'flex', alignItems:'center', gap:'0.7rem', marginBottom:'0.8rem', color:textColor,
    }}>
      {children}
...
export default function Preview({ S }) {
  ...
  const h1Style = {
    fontFamily: S.df, color: S.text,
    fontStyle: S.italic ? 'italic' : 'normal',
    fontSize: 'clamp(2.2rem, 4vw, 3.8rem)', fontWeight:900, lineHeight:1.03, marginBottom:'1rem',
    ...(S.gradText ? {
      ...
      <div>
        ...
        >
          <div
            ...
            className="flex justify-center items-center gap-1 text-lg font-bold"
          >
            <img
              src="/logo.png"
              alt="Brand Logo"
