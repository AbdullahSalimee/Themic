  ...
  return (
    ...
    >
      <div className={`db ${style.dbClass}`} style={{ pointerEvents: 'none' }}>
        {style.label.toUpperCase().split(' ')[0]}
      </div>
      <div style={{
        fontSize: '0.52rem', letterSpacing: '1.5px', textTransform: 'uppercase',
        color: 'var(--ui-muted)', fontFamily: "'DM Mono', monospace",
      }}>{style.label}</div>
    </div>
  );
}
...
export default function ComponentsTab({ S, update }) {
  const [gap,   setGap]   = useState(S.gap   ?? 2.5);
  const [pad,   setPad]   = useState(S.pad   ?? 1.2);
  const [pvpad, setPvpad] = useState(S.pvpad ?? 2.5);
  ...
  return (
    <div>
      ...
      <Section icon="↔" title="Spacing & Density">
        <Slider label="Content Gap" value={gap} min={1} max={5} step={0.5}
          displayValue={gap + 'rem'}
          onChange={v => { setGap(v); update({ gap: v }); }} />
        <Slider label="Card Padding" value={pad} min={0.5} max={2.5} step={0.25}
          displayValue={pad + 'rem'}
          onChange={v => { setPad(v); update({ pad: v }); }} />
        <Slider label="Preview Padding" value={pvpad} min={1} max={5} step={0.5}
          displayValue={pvpad + 'rem'}
          onChange={v => { setPvpad(v); update({ pvpad: v }); }} />
      </Section>
    </div>
  );
}
